...
5. Hold every comment in the diff to `docs/agents/comments.md`: flag lineage, and a citation that names no constraint the code cannot show.
6. Report only actionable findings, ordered by severity, with file and line evidence.
7. State residual risks and checks not run. If no findings remain, say so explicitly.
