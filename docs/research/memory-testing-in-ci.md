...
`console` egui application against the repository's existing two-tier CI. It proposes gates and
...
Tick runs on a musical clock, `orcvs` re-reads the Source for every Render Frame, and `console` is an
...
- An integration test is its own binary, so `#[global_allocator]` there cannot reach the shipped
  `console` binary or any other test target.
...
`console` is covered natively today — forty-seven unit tests across `console.rs`, `grid_viewport.rs`, `style.rs`, `midi.rs`, `persistence.rs`, and `diagnostics.rs` — while `console/tests/wasm.rs` stays `#![cfg(target_arch = "wasm32")]` and covers the web startup path alone. Those tests already construct the application headlessly: `console_over` builds a `Console` through `eframe::CreationContext::_new_kittest`, and the UI functions are driven a frame at a time through `egui::Context::run_ui`. What is missing is the loop. Nothing drives `eframe::App::update` repeatedly, and that is what the growth assertion needs:
...
handle re-created per frame. Nothing asserts that today: the existing tests check what a single frame produces, never what survives across many.

Two integration risks stood between this and the pull-request tier. The first is settled; the second is not:

- `Console::new(cc)` opens MIDI through `midir` on the desktop targets, so a headless runner with no ALSA sequencer exercises the constructor's failure path. That path is graceful: `MidiDeviceSelection::refresh_destinations` stores the error message in `status` and leaves the destination list empty, and the native tests that call `Console::new` prove it does not panic. No constructor seam is needed.
...
Sample it in `console/tests/wasm.rs` after a warm-up run, drive the app through a long sequence of
...
2. `egui_kittest` native test for `console` with the steady-state growth assertion, after resolving
   the MIDI-constructor and windowless-eframe questions above.
3. Linear-memory growth assertion in `console/tests/wasm.rs`, running in the merge tier.
