...
A Portal is one internal Cell destination resolved during a Tick against working Source under [ADR 0031](0031-evaluate-turns-against-working-source.md), not a language value or persistent state. A Portal may resolve any Cell; future Cell-addressing models, including an infinite canvas, can therefore change destination resolution without changing Function evaluation or Tick Plan commit semantics.
