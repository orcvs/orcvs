...
2. Start and retune failures, as one tier. A refused start or retune leaves the engine as it was, so the play state and BPM the Panel shows already say it did not happen. `Orcvs` hands neither refusal to its own caller, so the diagnostic is the only message about it.
