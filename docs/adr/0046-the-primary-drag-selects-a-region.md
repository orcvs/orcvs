...
The console gains a Region: a rectangle of Positions within the Grid, spanned from an anchor Cell to the Cursor. A Region always exists; when the anchor and the Cursor are the same Cell it is that one Cell, which is the ordinary state. The Cursor stays one Cell, at the Region's live end, so every rule that follows the Cursor — the Pan that brings it into view, the Cursor Effect — still names one Cell.
...
**The keyboard follows the spreadsheet.** Shift with an arrow moves the Cursor and keeps the anchor; a bare arrow collapses the Region and moves; command `A` spans the whole Grid; Escape collapses the Region onto the Cursor. Moving the Cursor past the anchor flips the rectangle, because the Region is the span between the two and has no corner of its own.
