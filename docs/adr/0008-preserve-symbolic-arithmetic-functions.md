...
The MIDI output Functions use `!>` for Raw Play, `!~` for Timed Play, `!%` for Monophonic Play, `!c` for Control Change, and `!b` for Pitch Bend. The `~` identifies the Tick lifetime of Timed Play. Monophonic Play preserves Orca's recognizable `%` glyph and owns one voice per output adapter and MIDI channel. Control Change and Pitch Bend accept direct hexadecimal MIDI bytes without scaling or clamping.
...
The Arithmetic Functions use `.+` for Addition, `.-` for Subtraction, `.|` for Absolute Difference, `.x` for Multiplication, `./` for Division, `.%` for Modulo, `.<` for Minimum, `.>` for Maximum, and `.=` for Equality. Multiplication uses the ASCII `x` because it remains legible beside the Bang Atom `**`.
