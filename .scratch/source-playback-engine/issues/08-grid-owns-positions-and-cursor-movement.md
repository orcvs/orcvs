...
**2026-08-26 — the duplication note above is closed, and `down` changed shape (review)**

The known duplication this ticket recorded is gone. Issue 10 took `cols`, `rows` and `count()` off `Opts`, so `App` no longer builds a Grid and an Opts from the same pair and the render loop reads the Grid. `Grid::count()` has a production caller now — the Source. The other two accessors never got one: renamed to `col_count()`/`row_count()` by issue 09 when `Grid::rows()` took the old name, and deleted on this branch, recorded there.

Criterion 3 still holds, by a different route. `Grid::down` no longer computes `(pos.y + 1).min(self.rows - 1)`; it delegates to `below` and clamps with `unwrap_or(pos)`, so the two cannot drift apart about where one row down is. For a Position this Grid minted the answer is identical, and `test_grid_moves_down_and_stops_at_the_bottom_row` plus the `below`/`down` assertion in `test_grid_answers_the_row_below_and_stops_past_the_bottom_row` are the net that says so. For a Position another Grid minted the two differ — noted on issue 19, which is where a Position not knowing its Grid belongs, not here. `up`, `left` and `right` are untouched.
