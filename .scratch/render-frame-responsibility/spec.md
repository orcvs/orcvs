...
Tickets `01`, `04`, and `08` are `ready-for-agent`. Tickets `02`, `03`, `05`, `06`, and
`07` are `resolved`. Ticket `09` is `needs-triage`. Maintainer triage on 2026-09-12 accepted
the four `needs-triage` tickets as filed, with two settlements recorded on the tickets:
