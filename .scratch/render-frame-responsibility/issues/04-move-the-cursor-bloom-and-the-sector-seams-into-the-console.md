...
**Settled by triage of ticket `01`: option (1).** `RenderFrame` answers the seam period and bloom
radius; presentation fields of `Opts` stay in `orcvs`. Moving them is the separate `Opts` drift the
...
**Status:** ready-for-agent — still blocked on `retired-glyph-vocabulary/01` (`needs-triage`).
`01`/`02`/`03` and `render-frame-derivation/01` are resolved on this branch; `02` of that effort is
still `needs-triage` with no early-out to carry. Do not edit `CONTEXT.md` here.
