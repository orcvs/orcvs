...
**2026-09-25 — resolved on `lang/narrow-api`.** `Stack::pop` and `MaybeAtom` are deleted, `pop_value` is `cfg(test)` for the tests that read what a Function left, and `Stack` itself is crate-private, since no dependent names it; `Expression::take_tokens`, `TypeError::{Function, Bang, Char, String}` and `ArgumentError::ExpectedFunction` are deleted. `Atom::Char` and `to_atom_char` are deleted; `Token::Char` stays as `orcvs`'s leftover-content token and refuses to decode. In `orcvs`, `RenderError::Unrepresentable` stays for `Encoding::literal` and as Tick planning's no-panic path, with tests restated over literal text plus a sweep proving every language value renders. `Interpreter::execute` is deleted and every caller uses `execute_function` (see 05); `Atom` returns to its natural two bytes, since the only measurement its width moved was `execute`'s. `tracing` is a dev-dependency. Commented-out code is gone.
