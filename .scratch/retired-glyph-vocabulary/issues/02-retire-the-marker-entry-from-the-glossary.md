...
- [x] The **Marker** entry at `CONTEXT.md:263-265` is gone from the glossary.
- [x] The `_Avoid_` list that was on it — "Guide, gridline, ruler dot" — is not silently lost.
      Whichever of those terms still threaten the **Sector Seam** entry `01` writes belong on that
      entry's `_Avoid_` line instead.
- [x] Nothing in the glossary claims a purely visual Glyph exists.
- [x] No other glossary entry is reworded, in particular not **Paint** (`CONTEXT.md:271-273`) or
      **Cursor** (`CONTEXT.md:255-257`).
- [x] The **Glyph** glossary entry is already gone (`typed-source-paint/06`); this line is
      satisfied by its absence. The glossary must not finish this ticket still naming Marker or
      Highlight as Cell classifications.
...
Deleted the **Marker** glossary block. Ticket `01` already rehomed its `_Avoid_` terms onto
**Sector Seam** (`_Avoid_: Marker, guide, gridline, ruler dot`); that list is the fold, so
nothing was duplicated. **Absence Marker** and that retired synonym are the only remaining
"Marker" mentions. **Glyph** was already gone; **Paint**, **Cursor**, and **Cursor Bloom**
were left untouched.
