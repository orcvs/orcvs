...
**2026-09-21 — reconciled by ADR 0053.**

The reconciliation this issue deferred is decided. [ADR 0053](../../../docs/adr/0053-one-theme-styles-the-whole-console.md) (which partly supersedes ADR 0051) makes every Source colour this issue exposed a value of a Theme: Tokens take base16 slots, and Diagnostic and Output Portal take named keys. The `Theme → Source colours` settings and the `source_paint` key are replaced rather than migrated, by `theming/06`. Sequence moves from its own colour to `base0F`. The Okabe–Ito values chosen here survive as the built-in Okabe–Ito Theme.
