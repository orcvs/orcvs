...
`RenderCell::output_portal(&self) -> bool` (`orcvs/src/render_frame.rs`) carries the fact per Cell, populated in `RenderFrame::derive` from `LanguageMap::output_portal_cells()` alongside the existing claim derivation. This is the smallest API `06` needs to add the fact to the paint decision.
