...
- [x] The decision is recorded: keep the span, or do not, and why.
- [x] *(Not applicable: not kept.)* If kept: where it lives, when it is discarded after an edit, and how it reads where a later producer overwrote part of an earlier one's span. Also whether it needs a `CONTEXT.md` entry and an ADR.
...
**2026-09-24 — triaged: `enhancement`, `wontfix`, out of `release/v1`. Decided: the span is not kept.** A Tick's admitted answer span stays inside the Tick Plan and is lost when writes flatten into per-Cell writes. Keeping it would be the first stored Tick outcome, would go stale after an edit, could name Cells a later producer now owns, and would not remove the Output Portal derivation. No second consumer needs it. The reasoning and the trigger for reconsidering are recorded in `.out-of-scope/keeping-a-ticks-answer-span.md`.
