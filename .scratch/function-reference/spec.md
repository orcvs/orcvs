...
The console opens on a complete Function reference instead of a blank Grid, so the language can be read and experimented with in place.
...
- The reference is shown when no Source is stored, and can be loaded on demand, replacing the current Source.
