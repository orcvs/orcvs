# 04 — Decide the light console palette

**What to build:** Decide the light chrome token values, record them in the console theme
documentation, and expose the theme switch once they are decided. Chrome only — a light Source
scheme is a base16 scheme like any other and ships through `06`, not here.

**Blocked by:** 03 — Port the console theme mechanism; 01 — Decide where Source colour authority
lives.
...
- [ ] Every light chrome token is named at an exact value, filling the same roles
      ...
      pairs, selection fill, the selection stroke while the caret is hidden, and the selection and
      Cursor stroke. The count is no longer twenty-two and this issue states what it is.
...
- [ ] `restyle-egui-console/02`'s near-black Cell background rule is restated for light rather
      than inherited. It is a dark-theme rule and does not transfer. Note that the Cell background
      is `base00` of the Source scheme under ADR 0051, so this rule constrains which schemes read
      well on a light chrome rather than constraining a chrome token.
- [ ] Glyph contrast is not decided here. It is a property of whichever scheme is selected, measured
      by `08`'s validator, and a light chrome does not change it.
- [ ] Light captures, wide and tall, are produced and reviewed the way
      `restyle-egui-console/03` reviews the dark ones.
...
- [ ] `console-testing/03` is extended to pin the light chrome values once they are decided. It does
      not pin glyph values, which `06` moves out of the palette.
