...
- [ ] Switching acceptance for a loaded Theme, handed from `04`: a loaded Theme is listed only by the picker of its parent's appearance, selecting it presents it for Source and chrome in the same frame, and its restored selection presents it on the next launch. Extend `console::kittest_tests`' switching tests, which already cover dark and light picker, mode, OS appearance and restored-reference changes against the test-only stand-ins `theme_selection::with_stand_ins` builds, so that they reach the loaded set through `SelectedThemes::new`'s shipped path rather than the stand-ins.
