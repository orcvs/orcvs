...
One Theme data model, represented in TOML only (JSON and YAML were dropped on
2026-09-24; ADR 0053 records why):
...
role backgrounds, direct TOML decoding and its strictness cases, format/version handling, optional-fill encoding,
...
features or lockfile change; issue `07` added `toml`, `serde_json` and `serde-saphyr`
to `console` (`serde_json` and `serde-saphyr` have since gone with JSON and YAML Themes), so its PR owed `mise run audit_deps` and a recorded rationale. Run workspace and doctest gates once before a PR.
