...
**What to build:** cargo-deny reports no duplicate-version warnings. Every remaining duplicate is held by an upstream crate's latest release, so each gets a skip entry that states the constraint:

- `windows-sys` 0.52, with its `windows-targets` 0.52 family: glutin 0.32 (latest 0.32.3 requires `^0.52`) and winit 0.30.
- `windows-sys` 0.60: `arboard` 3.6 accepts `>=0.52, <0.61`, and cargo resolves that open range to its greatest version (see 04).
