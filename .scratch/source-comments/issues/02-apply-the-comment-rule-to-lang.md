...
- [ ] No comment in `lang/` narrates what the code replaced, retired, relocated or used to do, or a rejected alternative, except as the thing a present-tense constraint forbids
...
- [ ] The scoped gate from `AGENTS.md` passes for `lang` and `orcvs`, and `RUSTDOCFLAGS="-D warnings" cargo doc --workspace --no-deps --locked` passes
