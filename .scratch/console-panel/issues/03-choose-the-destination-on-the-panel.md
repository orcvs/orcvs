...
**What to build:** The output destination is chosen from the Panel: a ComboBox and Refresh. The MIDI menu is gone. One destination. Empty copy is `None`. The first discovered destination is selected when nothing is. On a build with no native MIDI backend, the destination Readout is disabled with that copy and Refresh is hidden.
...
- [x] When the last discovery returned no destinations, the copy is `None`.
...
- [x] On a target or build with no native MIDI backend, the destination Readout is shown disabled as `None` and Refresh is hidden. BPM, Tick, and Run Clock remain.
- [x] Destination is not persisted. A new launch auto-selects the first destination again when one exists.

## Answer

Destination is an `egui::ComboBox` on the bottom Panel after Run Clock, with a Refresh button that calls `MidiDeviceSelection::refresh_destinations`. The MIDI menu is gone. Empty copy is the named constant `OUTPUT_NONE` (`"None"`), used by `destination_selected_text` / `destination_presentation` and the ComboBox.
