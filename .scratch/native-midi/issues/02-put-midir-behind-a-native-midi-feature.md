...
- [x] With the feature enabled, dependency resolution, behaviour, and the public surface are unchanged from today.
...
- [x] Verification exercises the feature disabled, enabled, and crossed with `persistence`, rather than assuming the two compose.
...
them to pass a gate, so the item is left unticked rather than ticked against two commands this
checkout did not execute. What ran in their place is in the report: `cargo fmt`, crate-scoped
