...
The dependency enables `eframe/accesskit` for test builds, which reaches `egui-winit/accesskit`. Not
`egui/accesskit`: no such feature exists in egui 0.36.1, where accesskit is a non-optional dependency
and the tree is generated by the runtime call `Context::enable_accesskit`. The feature matters here
only because `console/Cargo.toml` sets `default-features = false` on `eframe` and omits it. Under
resolver 3 a dev-dependency's features do not reach an ordinary build, so the shipped binary is
unchanged; confirm this with `cargo tree` rather than assuming it.
