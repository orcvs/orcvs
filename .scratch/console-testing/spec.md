...
The dependency enables `eframe/accesskit` for test builds, which reaches `egui-winit/accesskit`. Not `egui/accesskit`: no such feature exists in egui 0.36.1 — `egui`'s manifest declares `accesskit` as an ordinary non-optional dependency and its `[features]` table has no entry for it, so accesskit is compiled into egui unconditionally and the tree is generated by the runtime call `Context::enable_accesskit` instead. The feature matters here only because `console/Cargo.toml` sets `default-features = false` on `eframe` and omits it. Under resolver 3 a dev-dependency's features do not reach an ordinary build, so the shipped binary is unchanged.
