...
- [ ] `Grid::columns()` and `Grid::rows()` are public and answer the counts.
- [ ] The position iterator is `Grid::positions_by_row()`.
- [ ] All fourteen iterator call sites follow, and all nine `cols()` ones. Every one is inside `orcvs`; nothing in `lang` or `console` calls either.
...
- [ ] `source/model.rs:659` asks for the count rather than counting the iterator.
- [ ] `Grid::cols()` no longer exists under that name.
