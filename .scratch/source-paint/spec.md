...
- `Paint::derive(FramePaint)` takes nothing else. No `Orcvs`, no egui `Context`. This is the decision the whole effort turns on: if the derive needs a `Context`, the twenty-two Context-building tests that accept a blank Source to dodge harness cost keep dodging. **Revised by `07`:** the Position range is part of the pair, and it is not a `Context` — it is two `Range<usize>` the viewport has already resolved the clip `Rect` into. **Revised on `source-paint-09`:** those two values reach derive as a `FramePaint`, so a test still names a range without building a viewport (`FramePaint::range`) and a range minted for another Grid is refused. The bar this decision sets is unmoved.
