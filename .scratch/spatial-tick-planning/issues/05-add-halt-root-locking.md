...
- [x] An active Halt establishes a dependency edge that locks the complete root one row south
      before that root executes. `CONTEXT.md` already states Halt this way.
- [x] Empty target is a no-op; occupied non-root target diagnoses.
- [x] A suppressed Halt does not lock its own target.
- [x] Halt is never revisited after its turn.
- [x] The lock is an ordering edge, so it never needs to reach an already-executed root. A lock
...
- [x] Multiple Halts and activations follow ADR 0032's dependency order, with Position breaking
      ties. ADR 0020 governs only each producer's own emission order and Cell-wise conflict
      resolution.
- [x] Halt answers an effect rather than a value, so ADR 0025's single construction point refuses
