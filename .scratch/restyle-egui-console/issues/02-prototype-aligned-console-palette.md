...
**What to build:** One decided, named console palette that `restyle-egui-console/03` can falsify a capture against. `console/src/style.rs` already holds the single `ConsolePalette` struct and the `PALETTE` const that every rendering colour comes from, and `console/src/theme.md` already records the 22 token values in hex. This issue makes that record the decision rather than a description: the palette below is the one the captures must match, token by token. Where the shipped code is the only evidence for a choice, this issue says so and leaves the choice open for `03` to settle against the prototype.
...
- [ ] Semantic colours are derived from one theme boundary rather than scattered rendering literals: the `ConsolePalette` struct and the `PALETTE` const in `console/src/style.rs`.
- [ ] Glyph classification remains responsible for Glyph colour choice while Grid rendering owns geometry.
- [ ] The 22 palette tokens hold exactly these values, and `console/src/theme.md` states the same ones:
...
- [ ] The `marker` token is recorded as a compile-time requirement, not a visual one, and no capture
      ...
      `console/src/style.rs:76` must still map it to `PALETTE.marker`. Nothing produces the variant on
...
- [ ] The console adds no gradients, no rounded tiles, no shadows, and no animation. `console/src/style.rs:130-142` already sets `CornerRadius::ZERO` on windows, menus, and every widget state, and `Shadow::NONE` on windows and popups; `console/src/style.rs:146` sets `animation_time: 0.0`. Any new rendering keeps all four true.
- [ ] The sector registration marks are kept: partial 0.75-pixel `+` marks at each 8 × 8 sector corner
      (`SECTOR_LINE_WIDTH` at `console/src/console.rs:19`) with arm strengths
...
- [ ] The Cursor focus matrix is proposed, not settled: four Cell-aligned square bands of width
      ...
      only record of it is `console/src/theme.md`, which describes the shipped code, so it cannot be
...
- [ ] `console/src/theme.md` is named in the issue trail as the decided record, so a later palette change is a documented change rather than a drift.
