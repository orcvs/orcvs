...
impl Paint {
    ...
    pub(crate) fn derive(frame: &RenderFrame, drawn: &VisiblePositions) -> Self {
        ...
        Self {
            ...
            // outside it — not because the Render Frame is vague about where
            // the Cursor is. `RenderFrame::derive` takes one selected Position
            // and asserts its Grid owns it, so every Render Frame still marks
            // exactly one Cell; the walk above simply need not reach it.
            cursor,
            cells,
        }
    }
