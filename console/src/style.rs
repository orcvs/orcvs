...
mod tests {
    ...
    fn semantic_glyph_colours_are_distinct_and_read_from_the_settings_value() {
        ...
        let function = painted(SourcePaint::Function, false, source_paint);
        let number = painted(
        ...
        let ordinary = painted(SourcePaint::Unclaimed, false, source_paint);
        let bang = painted(SourcePaint::Bang, false, source_paint);
        let comment = painted(SourcePaint::Comment, false, source_paint);
    ...
    fn an_operand_cell_of_each_declared_token_is_tinted_with_its_own_colour() {
        ...
        for (paint, colour) in [
            ...
            let visuals = painted(paint, false, source_paint);
            assert_eq!(
                visuals.background,
    ...
    /// do with `0%`. Atom and Sequence are Pending because that is the only
    /// shape a Source produces for them (see [`operand`]), and a Pending
    /// operand slot still tints at any non-zero strength, so they carry the
    /// same weight here that a Valid Number does.
    ...
    fn zero_percent_fill_tint_paints_no_tint() {
        ...
        *source_paint.fill_tint_mut() = 0;
        ...
        for paint in [
            ...
            let visuals = painted(paint, false, source_paint);
            assert_eq!(visuals.background, None, "{paint:?} was tinted at 0%");
        }
    }
    ...
    /// strength being zero. A Leftover Char is not a fourth case here: it is
    /// Unclaimed, so it is exactly the [`SourcePaint::Unclaimed`] case
    /// below, not a fact of its own.
    ///
    ...
    fn an_invalid_operand_draws_diagnostic_but_keeps_its_declared_tint() {
        ...
        assert_eq!(
            ...
            "an Invalid Number Cell still tints as Number"
        );
    ...
    fn a_pending_operand_keeps_its_declared_colour_rather_than_diagnostic() {
        ...
        let pending = painted(
            operand(Token::Number, OperandState::Pending),
            false,
            source_paint,
        );
    ...
    fn output_portal_paints_over_an_unclaimed_and_a_bound_operand_cell() {
        ...
        for paint in [
            ...
            let visuals = painted(paint, true, source_paint);
            assert_eq!(
                visuals.foreground,
                source_paint.output_portal(),
    ...
    fn output_portal_keeps_the_bang_glyph_colour_but_takes_its_own_tint() {
        ...
        let ordinary_bang = painted(SourcePaint::Bang, false, source_paint);
        let portal_bang = painted(SourcePaint::Bang, true, source_paint);
    ...
    /// the Function that stands on a Cell; this layer cannot and need not
    ...
    fn a_bound_function_spelling_wins_over_output_portal() {
        ...
        let ordinary = painted(SourcePaint::Function, false, source_paint);
        let overlapped = painted(SourcePaint::Function, true, source_paint);
