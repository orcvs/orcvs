...
- Keep test-only inputs out of shipped code: no shipped function takes a parameter, or reaches a
  branch, that only a test populates. A test needing an input production cannot produce constructs
  it below the shipped entry point, in a test-only item beside the shipped one rather than through
  a seam cut into it.
